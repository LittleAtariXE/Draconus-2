...
from .servers.robbo_server import RobboTcpServer



class ClientHandler:
    def __init__(self, client_id: int, conn_obj: object, addr_obj: object, server: object, central: object, options: dict = {}):
        self.ID = client_id
        self.conn = conn_obj
        self.addr = addr_obj
        self.server = server
        ...
        self.encode_key = options.get("ENCODE_KEY")
    ...
    def send_file(self, fname: str, **kwargs) -> None:
        self.server.send_file(self, fname, **kwargs)
    
    def recive_file(self, **kwargs) -> None:
        self.server.recive_file(self, **kwargs)
...
class Central:
    ...
    def add_new_connection(self, conn_obj: object, addr_obj: object, server: object, options: dict = {}) -> None:
        with self.lock_client_id:
            cli_id = self.client_ID
            self.client_ID += 1
        new = ClientHandler(cli_id, conn_obj, addr_obj, server, self, options)
        with self.lock_clients:
            self.clients[str(new.ID)] = new
        self.msg("msg", f"New connection: <ID:{new.ID}> {new.addr[0]}:{new.addr[1]}", sender=server.draco_name)
    ...
    def sendFileToClient(self, handler_id: Union[int, str], data: any) -> None:
        handler = self.clients.get(str(handler_id))
        if not handler:
            self.msg("error", f"[!!] ERROR: Client Handler: '{handler_id}' is not connected. [!!]")
            return
        handler.send_file(data)
