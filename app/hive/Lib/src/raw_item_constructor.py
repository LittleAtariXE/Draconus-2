...
from .items_src.raw_config_item import RawConfigItem
...
class RawItemConstructor:
    ...
    def buildRawItem(self, raw_mod_info: object) -> Union[object, None]:
        ...
        match raw_mod_info.itemType:
            ...
            case "config":
                new = self.process_item(raw_mod_info, RawConfigItem)
            case _:
    ...
    def process_item(self, raw_mod_info: object, template_object: object) -> object:
        ...
        for head_line in raw_mod.load_item_data():
            ...
            match head[0]:
                ...
                case "VarOpt":
                    self.add_var_options(raw_mod, head[1:])
                case "foodAdd":
                ...
                case "codeOFF":
                    raw_mod.codeOFF = self.fGetBool(head[1])
    ...
    def add_var_options(self, raw_mod: object, head: list) -> None:
        var = raw_mod.Var.get(head[0])
        if not var:
            return
        var.update_option(head[1], head[2])
