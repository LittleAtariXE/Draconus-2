...
class Coder:
    ...
    def renderSingleTemplate(self, code: str, var: dict = {}) -> str:
        try:
            fcode = Template(code)
            fcode = fcode.render(**var, shTOOL=self.TEMP_shadow, asmTOOL=self.TEMP_nasm, randTOOL=self.TEMP_random, pyTOOL=self.TEMP_python, scTOOL=self.TEMP_shellcode, cTOOL=self.TEMP_c)
            return fcode
