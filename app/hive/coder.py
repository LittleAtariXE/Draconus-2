...
from .template_tools.c_temp import CTemplate
from .template_tools.temp_conn import ConnTemplate

from .pre_render_tools.prt_master import PreRenderTools
...
class Coder:
    def __init__(self, queen: object, raw_worm_builder: RawWormBuilder):
        ...
        self.TEMP_c = CTemplate(self)
        self.TEMP_conn = ConnTemplate(self)

        # Pre Render Var
        self.preVAR = PreRenderTools(self)
    ...
    def _build_extra_var(self) -> dict:
        ...
        extra["_EMPTY_VAR"] = WORM_VARIABLE_DEFAULT_EMPTY_VALUE
        return extra
    
    def _build_pre_var(self, var: dict, var_name: tuple) -> tuple:
        out = []
        for v in var_name:
            out.append(var[v])
        return tuple(out)
    
    def build_pre_var(self, var: dict) -> dict:
        pvar = {}
        for mod in self.raw_builder.wormAllMods:
            for k, i in mod.preRT.items():
                args = self._build_pre_var(var, i[1])
                out = self.preVAR.getTool(i[0], *args)
                pvar[k] = out
        return pvar
    ...
    def renderSingleTemplate(self, code: str, var: dict = {}) -> str:
        try:
            fcode = Template(code)
            fcode = fcode.render(**var, shTOOL=self.TEMP_shadow, asmTOOL=self.TEMP_nasm, randTOOL=self.TEMP_random, pyTOOL=self.TEMP_python, scTOOL=self.TEMP_shellcode, cTOOL=self.TEMP_c, connTOOL=self.TEMP_conn)
            return fcode
    ...
    def buildPythonCode(self, raw_lib_item_list: list, work_output_dir: str, master_file_name: str) -> tuple(list, str):
        ...
        for rl in raw_lib_item_list:
            # "payload" code will not be added
            if rl.itemType == "payload":
                continue
            if rl.lang:
