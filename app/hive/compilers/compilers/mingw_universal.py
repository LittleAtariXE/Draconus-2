...
class MinGW_All:
    ...
    def compileMod(self, raw_exe: RawExe, comp: RawCompiler) -> RawExe:
        ...
        match raw_exe.master_module.fileType:
            ...
            case "coff":
                self.compile_COFF(raw_exe, comp)
            case _:
    ...
    def compile_COFF(self, raw_exe: RawExe, comp: RawCompiler) -> RawExe:
        commands = comp.conf.get("BUILD_COFF")
        if not commands:
            self.msg("error", "[!!] ERROR: Missing Linker instructions. [!!]", sender=self.name)
            return raw_exe
        for cmd in commands:
            ...
            self.ccmd(cmd)
        self.msg("msg", "Building Complete", sender=self.name)
        self.msg("msg", "Update new config for COFF module.", sender=self.name)
        raw_exe.final_output_name = comp.conf.get("__COFF_SRC_FILE_NAME")
        raw_exe.master_module.addNameComp = True
        return raw_exe
