...
from .tools.pip_update import PipExtraLibrary
...
class MasterCompiler:
    def __init__(self, queen: Queen):
        ...
        ####################################################################
        self.PIP = PipExtraLibrary(self)
        self.PIP_link_name = "Python_Library"
        self.PIP_link_path = os.path.join(self.queen.DIR_LINKS, self.PIP_link_name)
        if not os.path.exists(self.PIP_link_path):
            os.symlink(self.PIP.extra_lib_path, self.PIP_link_path)
