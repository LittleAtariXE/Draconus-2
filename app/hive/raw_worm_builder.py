...
class RawWormBuilder:
    ...
    def wormGetModCompiler(self, owner_name: str) -> Union[RawLibItem, None]:
        mod_comp = self.RAW.mods_compiler.get(owner_name)
        if not mod_comp:
            mod_comp = self.RAW.payloads_compiler.get(owner_name)
        return mod_comp
    ...
    def addSupportFile(self, sfile_name: str, target_name: str, owner: RawLibItem) -> None:
        ...
        # self.RAW.sfiles[target_name] = sfile
    ...
    def addModCompiler(self, raw_info: object, target: Union[str, object], payload_var_name: str = None) -> None:
        if isinstance(target, str):
            target = self.get_target(target)
            if not target:
                self.msg("error", "[!!] ERROR: Target module does not exists [!!]", sender=self.name)
        ...
        comp = self.raw_constructor.buildRawItem(raw_info)
        comp.owner = target
        # The PAYLOAD compiler assumes a variable name for "payload".
        if payload_var_name:
            self.RAW.mods_compiler[payload_var_name] = comp
        else:
    ...
    def addPayloadCompiler(self, raw_info: object, target: Union[str, object], var_name: str = None) -> None:
        if isinstance(target, str):
            target = self.get_target(target)
            if not target:
                self.msg("error", "[!!] ERROR: Target module does not exists [!!]", sender=self.name)
        ...
        comp.owner = target
        if var_name:
            self.RAW.payloads_compiler[var_name] = comp
        else:
            self.RAW.payloads_compiler[target.name] = comp
        self.msg("msg", f"Add special compiler: '{comp.name}' to payload: '{target.name}' successfull.", sender=self.name)
        self.check_depediences(comp)
    ...
    def addPayload(self, raw_info: object, target: Union[str, object, None]) -> None:
        ...
        self.check_depediences(payload, target.name)
    ...
    def check_depediences(self, raw_mod: RawLibItem, payload_var_name: str = None) -> None:
        ...
        for rmod in raw_mod.reqMod:
            if rmod in self.RAW.modules.keys():
                # add owner
                self.RAW.modules[rmod].owner = raw_mod
                continue
            # check support mods
        ...
        # NEW check support file
        for sup_name, sup_code_name in raw_mod.inFile.items():
            if sup_name in self.RAW.sfiles.keys():
                # update owners
                self.RAW.sfiles[sup_name].owner = raw_mod
            else:
                self.addSupportFile(sup_name, sup_code_name, raw_mod)
        ...
        if raw_mod.payloadCompiler:
            comp = self.getLibItem("compiler", raw_mod.payloadCompiler)
            if not comp:
                self.msg("error", f"[!!] ERROR: Compiler: {raw_mod.payloadCompiler} does not exists in library. [!!]", sender=self.name)
            else:
                self.addPayloadCompiler(comp, raw_mod, payload_var_name)
    ...
    def showModules(self, no_separator: bool =  False) -> None:
        ...
        for mod in self.RAW.modules.values():
            ...
            if mod.itemType == "support":
                mod_tag.append("sM")
            owners = ""
            for ow in mod.owners:
                owners += f"<{ow.name}> "
            tab["data"].append([mod.name, self._build_info_tags(mod_tag), owners, mod.info])
