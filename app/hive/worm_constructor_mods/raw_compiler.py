...
class RawCompiler:
    ...
    def _buildBasicCompilerVar(self) -> dict:
        ...
        mlib, mlib_s = self._buildExtraModLib()
        cvar["__LINKER_EXTRA_MOD_FILE"] = mlib
        cvar["__LINKER_EXTRA_MOD_FILE_STR"] = mlib_s
        ...
        cvar["__COFF_SRC_FILE_NAME"] = ""
        cvar["__BUILD_COFF_FLAG"] = False
    ...
    def _buildExtraModLib(self) -> tuple(list, str):
        libs = self.raw_exe.master_module.compLib
        libs_str = ""
        for lib in libs:
            libs_str += f"-{lib} "
        return (libs, libs_str)
    
    def buildCoffName(self, coff_name: str) -> str:
        fpath = self.conf.get("__FINAL_OUTPUT_FNAME")
        cpath = fpath.replace(".", "_")
        self.conf["__COFF_SRC_FILE_NAME"] = f"{cpath}.o"
        return cpath
    
    def updateConfig(self, json_conf: Union[str, dict]) -> bool:
