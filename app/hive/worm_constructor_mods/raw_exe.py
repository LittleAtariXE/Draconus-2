...
class RawExe:
    ...
    @property
    def getChildMods(self) -> list:
        return self.sort_modules["child"]
    ...
    # New Version for new OWNER mods
    def _check_req_modules(self) -> list:
        crm = set()
        for raw in self.master_raw.allRawExe:
            if self.master_module in raw.master_module.owners:
                if raw.master_module.addNameComp:
                    crm.add(raw.final_output_name)
        return list(crm)
    ...
    def _check_req_files(self) -> list:
        crf = set()
        for mod in self.master_raw.allMods:
            if mod.itemType == "sfile":
                if self.master_module in mod.owners:
                    if mod.addNameComp:
                        name = mod.owner.inFile.get(mod.name)
                        if not name:
                            crf.add(mod.name)
                        else:
                            crf.add(name)
    ...
    def updateProcessCode(self, data: any):
        ...
        if isinstance(data, str):
            ...
            bdata = base64.b64encode(data).decode("ascii")
            self._process_code = bdata
    ...
    def update(self) -> None:
        ...
        # update parent raw_exe links
        for mod in self.modules:
            mod.raw_exe_parent = self
