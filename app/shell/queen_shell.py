import click
import os
from click_shell import shell
from termcolor import cprint


class QueenShell:
    def __init__(self, main_shell: object, commander: object):
        self.C2 = commander
        self.Queen = self.C2.Queen
        self.main_shell = main_shell

        self.pSort = self.main_shell.msg_sorter.sort_Text
        self.pTitle = self.main_shell.msg_sorter.make_Title
        self.pSort_set = self.main_shell.msg_sorter.set_text_len_first
        self.pSort_restore = self.main_shell.msg_sorter.restore_text_len_first
        self.pError = self.main_shell.msg_sorter.message_error
        self.sHelp = self.main_shell.help

        self.enter_first_time = True

    
    def exitHiveShell(self, *args, **kwargs) -> None:
        cprint("Exit Hive Shell", "yellow")
    
    def enterShell(self) -> None:
        if self.enter_first_time:
            self.Queen.enter()
            self.enter_first_time = False
    

    def shellBuild(self) -> object:

        @shell(prompt=f"[Queen] >>", intro="------ Welcome To Hive ! Put help for commands list ------- ", on_finished=self.exitHiveShell)
        def hiveShell() -> None:
            self.enterShell()
        
        @hiveShell.command()
        def help() -> None:
            self.pTitle("  Hive Help  ")
            self.pSort("clr, clear", "Clear screen.")
            self.pSort("exit", "Exit Hive Shell.")
            self.pSort("rebuild", "Clear Worm Constructor. Start Empty Template")
            self.pSort("show [type]", "Display all 'types' items in Library. Ex: 'show worm', 'show module'")
            self.pSort('var [-option] [name] "[value]"', "Add, set variable to worm. See 'var --help'")
            self.pSort('cvar [name] [value]', "Add, set compiler variable to worm. See 'cvar --help'")
            self.pSort("add [type] [name]", "Adds the selected module type to the worm. Ex: 'add module RawTcp', 'add worm Montezuma'")
            self.pSort("worm", "Show worm config. All loaded Modules, Variables etc.")
            self.pSort("name [new_name]", f"Set worm name. Actual {self.Queen.raw_builder.worm_name}")
            self.pSort("icon [-option]", "Set icon to worm. See 'icon --help'")

            print("\n")
            self.pTitle("  Module Types:  ")
            self.pSort_set(15)
            self.pSort("worm", "The main template; required to add first. Its choice determines whether you build an injector, shellcode, DLL, or something else.")
            self.pSort("", "Different extra modules can be attached to a worm depending on the chosen template.")
            self.pSort("module", "Various modules or libraries that add functionality. Some worms allow additional modules to be added.")
            self.pSort("support", "Modules and libraries added automatically when required by a worm or another module.")
            self.pSort("payload", "Various kinds of payloads, small and large, implemented in different languages. If a worm supports adding payloads, they are listed here.")
            self.pSort("shadow", "Code obfuscation. Additional modules used to obfuscate code.")
            self.pSort("food", "'Food' for the worm. Contains assorted data like text databases for obfuscators, random game names, ready-made shellcodes, and other resources.")
            self.pSort("", "Worms automatically use food when needed, but you can change items or assign them to a variable 'var'.")
            self.pSort("Wprocess", "The lifecycle a worm goes through to reach its final form. These are added automatically; only modify them if you really know what you are doing.")
            self.pSort("scode", "Template for creating shellcodes.")
            print("\n")
            self.pSort_restore()
        
        @hiveShell.command()
        def clr() -> None:
            os.system("clear")
        
        @hiveShell.command()
        def clear() -> None:
            os.system("clear")
        
        @hiveShell.command()
        @click.argument("item_type")
        def show(item_type):
            self.Queen.showItems(item_type)
        
        @hiveShell.command()
        @click.argument("name", required=False)
        @click.argument("value", required=False)
        @click.option("--food", "-f", required=False, is_flag=True, help="Add Food to variable. Ex: 'var -f [var_name] [food_name]")
        @click.option("--types", "-t", required=False, help="Set type of variable. Ex: str, int, list etc.")
        @click.option("--help", "show_help", required=False, is_flag=True, help="Show help.")
        def var(name, value, food, types, show_help):
            if show_help:
                self.sHelp.HIVE_VARIABLES_HELP
                return
            if not name or not value:
                self.pError("ERROR: name and value is required.")
                return
            if food:
                self.Queen.addVariableAsFood(name, value)
                return
            if types:
                self.Queen.addVariable(name, value, types)
            else:
                self.Queen.addVariable(name, value)
        
        @hiveShell.command()
        @click.argument("name", required=False)
        @click.argument("value", required=False)
        @click.option("--help", "show_help", required=False, is_flag=True, help="Show help.")
        def cvar(name, value, show_help) -> None:
            if show_help:
                self.sHelp.HIVE_COMP_VARIABLES_HELP
                return
            if name and value:
                self.Queen.addCompVariable(name, value)
            else:
                self.pError("ERROR: name and value is required.")

        @hiveShell.command()
        @click.option("--help", "show_help", required=False, is_flag=True, help="Show help.")
        @click.option("--set_icon", "-s", required=False, help="Set icon to worm. Ex: 'icon -s rad1.ico")
        @click.option("--list", "icon_list", "-l", required=False, is_flag=True, help="Show icons list.")
        @click.option("--xlist", "-xl", required=False, is_flag=True, help="Show icons list and information.")
        def icon(show_help, set_icon, icon_list, xlist) -> None:
            if show_help:
                self.sHelp.HIVE_ICON_HELP
                return
            if set_icon:
                self.Queen.setWormIcon(set_icon)
                return
            if icon_list:
                self.Queen.showIconList()
            if xlist:
                self.Queen.showIconInfo()
        
        @hiveShell.command()
        @click.option("--no_compile", "-nc", required=False, is_flag=True, help="It does not perform compilation. It only creates a code file.")
        @click.option("--compiler", "-c", required=False, help="Name of compiler used for compilation.")
        @click.option("--payload", "-p", required=False, help="Build your worm as payload. Put ready module to library.")
        @click.option("--as_food", "-af", required=False, help="Build worm as Food. Put ready module to food section.")
        @click.option("--help", "-h", "show_help", required=False, is_flag=True, help="Show help.")
        def build(no_compile, compiler, payload, as_food, show_help):
            if show_help:
                self.sHelp.HIVE_BUILD_COMPILE_HELP
                return
            opt = {}
            if no_compile:
                opt["NO_COMPILE"] = True
            if payload:
                opt["BUILD_PAYLOAD"] = True
                opt["PAYLOAD_INFO"] = payload
            if as_food:
                opt["BUILD_FOOD"] = True
                opt["FOOD_INFO"] = as_food
            self.Queen.buildWorm(opt)
            


        
        @hiveShell.command()
        @click.argument("types")
        @click.argument("name")
        def add(types, name) -> None:
            self.Queen.addWormItem(types, name)
        
        @hiveShell.command()
        def worm() -> None:
            self.Queen.showWorm()
        
        @hiveShell.command()
        @click.argument("worm_name")
        def name(worm_name) -> None:
            """Set name for worm."""
            self.Queen.setWormName(worm_name)
        

        @hiveShell.command()
        def rebuild() -> None:
            self.Queen.resetWormTemplate()

        


        

        return hiveShell
    
    def Start(self) -> None:
        hshell = self.shellBuild()
        hshell()


