...
class SHELL_HELP_MESSAGES:
    ...
    def DRACONUS_SERVER_TYPE_HELP(self) -> None:
        ...
        self.pText("")
        self.pSort("rtcp", "Robbo TCP - A TCP socket server. It encrypts messages using an XOR key received from the client.")
        self.pSort("", "Compatible with other clients based on the same protocol. In addition to sending commands, it supports both file upload and download.")
        self.pSort("", "File sending is limited to files located in the 'INPUT' directory.")
        self.pText("")
        self.pTitle("  Example:  ")
